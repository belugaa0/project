...
function updateTonBalancesUI(userOrId) {
  ...
  userRef.get().then((doc) => {
    ...
    const internal = data.internalTonBalance || 0;

    const tonElem = document.getElementById("tonBalance");
    const internalProfileElem = document.getElementById("internalTonBalanceProfile");
    const internalNavbarElem = document.getElementById("internalTonBalanceNavbar");

    if (tonElem) tonElem.textContent = ton.toFixed(2);
    if (internalProfileElem) internalProfileElem.textContent = internal.toFixed(2);
    if (internalNavbarElem) internalNavbarElem.textContent = internal.toFixed(2);
  });
}
